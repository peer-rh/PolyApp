...
class SubchapterPage extends ConsumerStatefulWidget {
  ...
  const SubchapterPage(this.id, this.onFinished,
      {this.onNextSubchapter,
      this.nextSubchapterTitle,
      this.static = true,
      Key? key})
      : super(key: key);
...
class SubchapterPageState extends ConsumerState<SubchapterPage> {
  ...
  Widget build(BuildContext context) {
    final userProgress = ref.watch(userLearnTrackProvider);
    ref.watch(subchapterProvider(widget.id)).when(
          data: (data) => setState(() {
            subchapter = data;
          }),
          error: (e, s) => print("Error loading subchapter: $e, $s"),
          loading: () {},
        );
    ...
    void goToLesson(int i) {
      ...
      onFinished() {
        Future(() {
          ref.read(userLearnTrackProvider).setStatus(subchapter!.id, lesson.id);
        });
      }
