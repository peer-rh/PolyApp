...
final staticAiChatLessonProvider =
  ...
  final collection = (id.startsWith("custom_"))
      ? ref.watch(userLearnTrackDocProvider).collection("custom")
      : ref.read(staticFirestoreDoc).collection("lessons");
...
final activeChatSession = ChangeNotifierProvider<ActiveChatSession?>((ref) {
  ...
  final chatLesson = ref.watch(staticAiChatLessonProvider(id));
  chatLesson.when(
      data: (d) {},
      error: (e, s) {
        print(e);
      ...
      loading: () {});
  final lesson = chatLesson.asData?.value;
